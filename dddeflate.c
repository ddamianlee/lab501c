	...
	*outlen = 0;
	...
	switch (dctx->state) {
	  ...
	  case TREES_LENREP:
		...
		//debug(("recv: codelen-extrabits %d/%d\n", rep - dctx->lenaddon,dctx->lenextrabits));
#ifdef ANALYSIS
	    if (analyse_level > 1)
...
int main(int argc, char **argv)
{
    ...
    while (--argc)
    {
        ...
        chandle = deflate_compress_new(DEFLATE_TYPE_BARE);
        dhandle = deflate_decompress_new(DEFLATE_TYPE_BARE);
        ...
        do
        {
            ...
            if(ret <= 0)
            {
                deflate_compress_data(chandle, NULL, 0, DEFLATE_END_OF_DATA, (void **)&outbuf, &outlen);
            }
            else
            {
                dlen += ret;
                deflate_compress_data(chandle, buf, ret, DEFLATE_NO_FLUSH, (void **)&outbuf, &outlen);
            }
            if(outbuf)
            {
                clen += outlen;
                err = deflate_decompress_data(dhandle, outbuf, outlen, (void **)&outbuf2, &outlen2);
                sfree(outbuf);
                if(outbuf2)
                {
                    if(outlen2)
