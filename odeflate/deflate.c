...
typedef block_state (*compress_func) OF((TOID(struct deflate_state) s, int flush));
...
local block_state deflate_stored OF((TOID(struct deflate_state) s, int flush));
local block_state deflate_fast   OF((TOID(struct deflate_state) s, int flush));
...
local unsigned read_buf          OF((TOID(struct z_stream) strm, Bytef *buf, unsigned size));
    ...
    int stream_size;
{
    ...
    if (D_RO(strm) == Z_NULL) return Z_STREAM_ERROR;
    ...
    POBJ_ZALLOC(pop, &s, struct deflate_state, sizeof(struct deflate_state));
    ...
    D_RW(strm)->overlay = (ushf *) POBJ_ZALLOC(pop, &s, struct z_stream, D_RO(s)->lit_bufsize * sizeof(ush)+2);

    D_RW(s)->pending_buf = D_RO(strm)->overlay;
    ...
    int flush;
{
    ...
    if (D_RO(strm)->avail_in != 0 || D_RO(s)->lookahead != 0 ||
        ...
        bstate = D_RO(s)->level == 0 ? deflate_stored(s, flush) :
                 D_RO(s)->strategy == Z_HUFFMAN_ONLY ? deflate_huff(s, flush) :
                 D_RO(s)->strategy == Z_RLE ? deflate_rle(s, flush) :
                 (*(configuration_table[D_RO(s)->level].func))(s, flush);
...
local block_state deflate_slow(pop, s, flush)
    ...
    int flush;
{
    TOID(struct z_stream) strm = D_RW(s)->strm;
    ...
    for (;;) {
        ...
        if (D_RO(s)->prev_length >= MIN_MATCH && D_RO(s)->match_length <= D_RO(s)->prev_length) {
            ...
            D_RW(s)->strstart++;
            D_RW(s)->lookahead--;
            if (D_RW(strm)->avail_out == 0) return need_more;
        } else {
...
#define FLUSH_BLOCK_ONLY(s, last) { \
   _tr_flush_block(s, (D_RO(s)->block_start >= 0L ? \
...
#define FLUSH_BLOCK(s, last) { \
   ...
   if (D_RO(strm)->avail_out == 0) return (last) ? finish_started : need_more; \
}
