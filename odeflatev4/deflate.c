...
#define INSERT_STRING(s, str, match_head) \
   (UPDATE_HASH(s, ws->ins_h, ws->window[(str) + (MIN_MATCH-1)]), \
    ...
    int stream_size;
{
    ...
    if (rs->window == Z_NULL || rs->prev == Z_NULL || rs->head == Z_NULL ||
    ...
    TOID(struct z_stream) strm;
{
    ...
    //POBJ_FREE(&rs->head);
    TRY_FREE(D_RW(strm), rs->head);
    TRY_FREE(D_RW(strm), rs->prev);

    //POBJ_FREE(&rs->prev);
    //POBJ_FREE(&rs->window);
    TRY_FREE(D_RW(strm), rs->window);
    ...
    TOID(struct deflate_state) s;
{
    ...
    /* CLEAR_HASH(s) */
    ws->head[rs->hash_size-1] = NIL; 
    memset((Bytef *)ws->head, 0, (unsigned)(rs->hash_size-1)*sizeof(*rs->head));
    ...
    TOID(struct deflate_state) s;
{
    ...
    do {
        ...
        if (rs->strstart >= wsize+MAX_DIST(s)) {
            memcpy(ws->window, rs->window + wsize, (unsigned)wsize - more);
        ...
        n = read_buf(pop, rs->strm, ws->window + rs->strstart + rs->lookahead, more);
        ws->lookahead += n;

        /* Initialize the hash value now that we have some input: */
        if (rs->lookahead + rs->insert >= MIN_MATCH) {
            uInt str = rs->strstart - rs->insert;
            ws->ins_h = rs->window[str];
            UPDATE_HASH(s, ws->ins_h, ws->window[str + 1]);
#if MIN_MATCH != 3
    ...
    if (rs->high_water < rs->window_size) {
        ...
        if (rs->high_water < curr) {
            ...
            if (init > WIN_INIT)
                init = WIN_INIT;
            memset(ws->window + curr, 0, (unsigned)init);
            ...
            memset(ws->window + rs->high_water, 0, (unsigned)init);
            ws->high_water += init;
        }
    }
    ...
    IPos cur_match;                             /* current match */
{
    ...
    register Bytef *scan = ws->window + rs->strstart; /* current string */
...
#else
    register Bytef *strend = ws->window + rs->strstart + MAX_MATCH;
...
#define FLUSH_BLOCK_ONLY(s, last) { \
   _tr_flush_block(pop, s, (rs->block_start >= 0L ? \
                   (charf *)&(ws->window[(unsigned)rs->block_start]) : \
                   (charf *)Z_NULL), \
    ...
    int flush;
{
    ...
    for (;;) {
        ...
        if (rs->prev_length >= MIN_MATCH && rs->match_length <= rs->prev_length) {
            ...
            _tr_tally_lit(s, ws->window[rs->strstart-1], bflush);
            if (bflush) {
    ...
    int flush;
{
    ...
    do {
        ...
        if (left) {
            if (left > len)
                left = len;
            pmemobj_memcpy_persist(pop, D_RW(ws->strm)->next_out, ws->window + rs->block_start, left);
    ...
    if (used) {
        ...
        if (used >= rs->w_size) {    /* supplant the previous history */
            ...
            memcpy(ws->window, D_RW(ws->strm)->next_in - rs->w_size, rs->w_size);
            ...
            if (rs->window_size - rs->strstart <= used) {
                ...
                ws->strstart -= rs->w_size;
                memcpy(ws->window, ws->window + rs->w_size, rs->strstart);
                if (rs->matches < 2)
                    ws->matches++;   /* add a pending slide_hash() */
            }
            memcpy(ws->window + rs->strstart, D_RW(ws->strm)->next_in - used, used);
            ws->strstart += used;
        }
        ws->block_start = rs->strstart;
    ...
    if (have) {
        read_buf(pop, rs->strm, ws->window + rs->strstart, have);
        ws->strstart += have;
    }
    if (rs->high_water < rs->strstart)
        ws->high_water = rs->strstart;
    ...
    if (left >= min_block ||
        ...
        last = flush == Z_FINISH && D_RO(rs->strm)->avail_in == 0 &&
               len == left ? 1 : 0;
        _tr_stored_block(pop, s, (charf *)rs->window + rs->block_start, len, last);
        ws->block_start += len;
        flush_pending(pop, rs->strm);
    }
    ...
    for (;;) {
        ...
        if (rs->match_length >= MIN_MATCH) {
...
#endif
            {
                ...
                ws->match_length = 0;
                ws->ins_h = rs->window[rs->strstart];
                UPDATE_HASH(s, ws->ins_h, rs->window[rs->strstart+1]);
#if MIN_MATCH != 3
                Call UPDATE_HASH() MIN_MATCH-3 more times
#endif
            ...
            _tr_tally_lit (s, rs->window[rs->strstart], bflush);
            ws->lookahead--;
            ws->strstart++;
        }
        if (bflush) FLUSH_BLOCK(s, 0);
    ...
    int flush;
{
    ...
    for (;;) {
        ...
        if (rs->lookahead >= MIN_MATCH && rs->strstart > 0) {
            scan = ws->window + rs->strstart - 1;
            prev = *scan;
            if (prev == *++scan && prev == *++scan && prev == *++scan) {
                strend = ws->window + rs->strstart + MAX_MATCH;
                do {
