...
/* Number of bytes after end of data in window to initialize in order to avoid
   memory checker errors from longest match routines */
        ...
        /* in trees.c */
void ZLIB_INTERNAL _tr_init OF((PMEMobjpool *pop, deflate_state *s));
int ZLIB_INTERNAL _tr_tally OF((deflate_state *s, unsigned dist, unsigned lc));
void ZLIB_INTERNAL _tr_flush_block OF((PMEMobjpool *pop, deflate_state *s, charf *buf,
                        ulg stored_len, int last));
void ZLIB_INTERNAL _tr_flush_bits OF((deflate_state *s));
void ZLIB_INTERNAL _tr_align OF((deflate_state *s));
void ZLIB_INTERNAL _tr_stored_block OF((PMEMobjpool *pop, deflate_state *s, charf *buf,
                        ulg stored_len, int last));
...
# define _tr_tally_dist(s, distance, length, flush) \
  { uch len = (uch)(length); \
    ...
    dist--; \
    D_RW(s->dyn_ltree)[_length_code[len]+LITERALS+1].Freq++; \
