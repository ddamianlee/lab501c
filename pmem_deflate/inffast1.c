...
unsigned start;         /* inflate()'s starting value for strm->avail_out */
{
    ...
    *D_RW(dmask) = (1U << rstate->distbits) - 1;
    ...
    const unsigned *distc = D_RO(dist);
    ...
    do {
      ...
      dolen:
          ...
          dodist:
            ...
            if ((*op_c) & 16) {                      /* distance base */
               ...
               (*distw) += (unsigned)(*hold_c) & ((1U << (*op_w)) - 1);
                ...
                if ((*distc) > (*op_c)) {                /* see if copy from window */
                   (*op_w) = (*distc) - (*op_c);             /* distance back in window */
                    if ((*op_c) > (*whave_c)) {
                        if (rstate->sane) {
                            wstrm->msg =
                    ...
                    if ((*wnext_c) == 0) {           /* very common case */
                        from += (*wsize_c) - (*op_c);
                        if ((*op_c) < (*lenc)) {         /* some from window */
                            (*lenw) -= (*op_c);
                            do {
                                *out++ = *from++;
                            } while (--((*op_w)));
                            from = out - (*distc);  /* rest from output */
                        }
                    }
                    else if ((*wnext_c) < (*op_c)) {      /* wrap around window */
                        from += (*wsize_c) + (*wnext_c) -(*op_c);
                       (*op_w) -= (*wnext_c);
                        if ((*op_c) < (*lenc)) {         /* some from end of window */
                            (*lenw) -= (*op_c);
                            do {
                                *out++ = *from++;
                            } while (--(*op_w));
                            from = wstate->window;
                            if ((*wnext_c) < (*lenc)) {  /* some from start of window */
                               (*op_w) = (*wnext_c);
                                (*lenw) -= (*op_c);
                                do {
                                    *out++ = *from++;
                    ...
                    from = out - (*distc);          /* copy direct from output */
            ...
            else if (((*op_c) & 64) == 0) {          /* 2nd level distance code */
                here = dcode[here.val + ((*hold_c) & ((1U << (*op_c)) - 1))];
                goto dodist;
            }
            else {
        ...
        else if (((*op_c) & 64) == 0) {              /* 2nd level length code */
            here = lcode[here.val + ((*hold_c) & ((1U << (*op_c)) - 1))];
            goto dolen;
        }
        else if ((*op_c) & 32) {                     /* end-of-block */
